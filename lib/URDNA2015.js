...
module.exports = class URDNA2015 {
  ...
  async main(dataset) {
    ...
    if(this.format === 'application/rdfjs') { // TBD: appropriate format name
      for(const quad of this.quads) {
        // 7.1) Create a copy, quad copy, of quad and replace any existing
        // blank node identifiers using the canonical identifiers
        // previously issued by canonical issuer.
        // Note: We optimize away the copy here.
        const canonicalizedQuad = {
          subject: this._componentWithCanonicalId(quad.subject),
          predicate: quad.predicate,
          object: this._componentWithCanonicalId(quad.object),
          graph: this._componentWithCanonicalId(quad.graph)
        };
        // 7.2) Add quad copy to the normalized dataset.
        normalized.push(canonicalizedQuad);
      }

      // 8) Return the normalized dataset
      // Note: unsorted set of quads instead of sorted N-Quads
      return new Set(normalized);
    } else {
      // Note: "application/n-quads" for default
      for(const quad of this.quads) {
        // 7.1) Create a copy, quad copy, of quad and replace any existing
        // blank node identifiers using the canonical identifiers
        // previously issued by canonical issuer.
